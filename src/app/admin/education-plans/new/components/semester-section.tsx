...
import type { EducationPlan } from "@/services/supabase/types";
...
export const SemesterSection = ({ subjects, defaultValue, search }: Props) => {
	const [semesters, setSemesters] = useState(
		defaultValue?.semesters.length ?? 1,
	);
		...
		<>
			<LabeledInput
				...
				onChange={(event) => {
					const { success, data } = z
						.number()
						.safeParse(Number(event.target.value)) as {
						success: boolean;
						data: number;
					};
			...
			<div className="flex w-full h-96 overflow-hidden ">
				<section className="px-3 flex flex-col py-1 rounded-md bg-itesus-secondary h-full w-96">
					<div className="flex border-b mb-3 py-1 justify-between gap-10">
				...
				>
					{Array.from({ length: semesters }).map((_, index) => (
						<article
							key={v4()}
							className="border px-3 py-1 rounded-md min-h-72 w-full"
						>
							<H2 className="border-b py-1 mb-3 text-white">
								Semestre {index + 1}
							</H2>
							...
							<MultiDragAndDrop
								...
								options={
									semesterItem
										.find((s) => s.number === index + 1)
										?.semester_subjects.map((ss) => ss.subjects?.name ?? "") ??
									[]
								}
								id={`${index + 1}`}
							/>
